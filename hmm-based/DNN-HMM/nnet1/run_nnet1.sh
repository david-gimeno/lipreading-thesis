...
for sp in 5; do
  ...
  steps/nnet/pretrain_dbn.sh --rbm-iter 3 --splice $sp $data_fmllr_dir/$train $output_dir/pretrain-dbn || exit 1;
  ...
  for klr in 0; do # 5 10; do
    ...
    steps/nnet/train.sh --proto-opts "--activation-type <"${actv}">" \
      ...
      $data_fmllr_dir/$train $data_fmllr_dir/dev $lang_dir ${ali_root}_${train} ${ali_root}_dev $nnet_dir || exit 1;
