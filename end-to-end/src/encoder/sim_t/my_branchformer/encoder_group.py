...
class EBranchformerEncoderGroup(torch.nn.Module):
    ...
    Args:
        ...
        num_layers (int): number of layers that compose the encoder group
    ...
    def forward(self, x_input, mask, cache=None):
        ...
        for j in range(0, self.num_layers):
            ...
            # Branch 1: Self Attention-based Branch
            if j == 0:
                # -- Pre-MHA Module
            ...
            if self.use_two_branches:
                if self.merge_method == "concat":
                    ...
                    self.weight_global.append(w1)
                    self.weight_local.append(w2)
    ...
    def post_mha(self, values, scores):
        ...
        value = self.attn.linear_v(values).view(n_batch, -1, self.attn.h, self.attn.d_k)
        ...
        p_attn = self.attn.dropout(scores)
        x = torch.matmul(p_attn, value)  # (batch, head, time1, d_k)
        x = (
            x.transpose(1, 2).contiguous().view(n_batch, -1, self.attn.h * self.attn.d_k)
        )  # (batch, time1, d_model)
